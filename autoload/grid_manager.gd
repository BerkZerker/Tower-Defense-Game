...
func _ready() -> void:
    ...
    # Connect to our own placement_indicator_ready signal
    connect("placement_indicator_ready", _on_placement_indicator_ready)
    
    if grid:
        _setup_grid()
        emit_signal("grid_manager_ready")

func _on_placement_indicator_ready(new_placement_indicator: Sprite2D) -> void:
...
func _draw_grid() -> void:
    ...
    for x in range(grid_width + 1):
        var line = Line2D.new()
        line.width = 1.0
        line.default_color = Color(0.5, 0.5, 0.5, 0.3)  # Subtle grid lines
